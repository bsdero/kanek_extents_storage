...
static void default_config( kes_cache_config_t *cfg) {
    /* small/fast edge sizing for tests */
...
static bool test_kes_cache_get_default_config(void) {
    ...
    TEST_ASSERT( edge.max_memory >= KES_CACHE_MIN_MEMORY,
                 "edge config meets min memory");
    TEST_ASSERT( server.max_memory >= KES_CACHE_MIN_MEMORY,
                 "server config meets min memory");
...
static bool test_kes_cache_create(void) {
    ...
    TEST_ASSERT( kes_cache_create( &cfg) == NULL,
                 "below-minimum max_memory rejected");

    default_config( &cfg);
    cfg.max_entries = KES_CACHE_MIN_ENTRIES - 1;
    TEST_ASSERT( kes_cache_create( &cfg) == NULL,
                 "below-minimum max_entries rejected");
...
static bool test_kes_cache_destroy(void) {
    ...
    TEST_ASSERT( kes_cache_destroy( NULL) == KES_ERROR_INVALID,
                 "NULL cache rejected");
...
static bool test_kes_cache_stop(void) {
    ...
    TEST_ASSERT( kes_cache_stop( NULL) == KES_ERROR_INVALID,
                 "NULL cache rejected");
...
static bool test_kes_cache_set_io_callbacks(void) {
    ...
    TEST_ASSERT( kes_cache_set_io_callbacks( cache, mock_read, mock_write,
                                             mock_sync)
                == KES_SUCCESS, "set callbacks success");
    TEST_ASSERT( kes_cache_set_io_callbacks( NULL, mock_read, mock_write,
                                             mock_sync)
...
static bool test_kes_cache_get_put_extent(void) {
    ...
    TEST_ASSERT( ((unsigned char *)buf)[0] == 0x42,
                 "loaded data matches backing store");
    ...
    TEST_ASSERT( kes_cache_get_extent( cache, NULL,
                                       &buf) == KES_ERROR_INVALID,
...
static bool test_kes_cache_pin_unpin(void) {
    ...
    TEST_ASSERT( kes_cache_pin_extent( cache, &id) == KES_SUCCESS,
                 "pin success");
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_pinned == 1,
                 "entries_pinned reflects one pin");
    ...
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_pinned == 1,
                 "still one pinned ENTRY (pin is refcounted)");

    TEST_ASSERT( kes_cache_unpin_extent( cache, &id) == KES_SUCCESS,
                 "first unpin success");
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_pinned == 1,
                 "still pinned after only one of two unpins");

    TEST_ASSERT( kes_cache_unpin_extent( cache, &id) == KES_SUCCESS,
                 "second unpin success");
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_pinned == 0,
                 "fully unpinned after matching unpin count");
...
static bool test_kes_cache_mark_dirty(void) {
    ...
    TEST_ASSERT( stats.entries_dirty == 1,
                 "entries_dirty reflects one dirty entry");

    /* idempotent: marking an already-dirty entry again must not
     * double-count */
    TEST_ASSERT( kes_cache_mark_dirty( cache, &id) == KES_SUCCESS,
                 "re-mark dirty success");
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_dirty == 1,
                 "entries_dirty unchanged on re-mark");
...
static bool test_kes_cache_flush_extent(void) {
    ...
    TEST_ASSERT( kes_cache_flush_extent( cache, &id) == KES_SUCCESS,
                 "flush of dirty entry");
    TEST_ASSERT( g_write_calls == writes_before + 1,
                 "write_extent callback actually invoked");
    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_dirty == 0,
                 "entries_dirty cleared after flush");

    kes_extent_id_t missing = make_id( 50);
    TEST_ASSERT( kes_cache_flush_extent( cache,
                                         &missing) == KES_ERROR_NOTFOUND,
...
static bool test_kes_cache_get_stats(void) {
    ...
    TEST_ASSERT( kes_cache_get_stats( cache, &stats) == KES_SUCCESS,
                 "get_stats success");
...
static bool test_kes_extent_equal(void) {
    ...
    TEST_ASSERT( kes_extent_equal( &a, &b) == true,
...
static bool test_kes_cache_reset_stats(void) {
    ...
    cache = kes_cache_create( &cfg);
    kes_cache_set_io_callbacks( cache, mock_read, mock_write, mock_sync);
    kes_cache_get_extent( cache, &id,
                          &buf);   /* one miss, one cached entry */
...
static bool test_kes_cache_invalidate(void) {
    ...
    TEST_ASSERT( kes_cache_invalidate( cache, &id) == KES_ERROR_BUSY,
                "invalidate refuses a still-referenced "
                "(ref_count > 0) entry");
    ...
    TEST_ASSERT( kes_cache_get_extent( cache, &id, &buf) == KES_SUCCESS,
                "id is fetchable again after invalidate "
                "(re-reads from backing store)");
...
static bool test_kes_cache_sync(void) {
    ...
    /* unreferenced, should be freed by sync */
    kes_cache_put_extent( cache, &id_a);
    kes_cache_get_extent( cache, &id_b, &buf);
    /* stays referenced -- must NOT be freed */
    kes_cache_mark_dirty( cache, &id_b);

    int writes_before = g_write_calls;
    TEST_ASSERT( kes_cache_sync( cache) == KES_SUCCESS, "sync success");
    TEST_ASSERT( g_write_calls == writes_before + 2,
                 "both dirty entries were flushed");

    kes_cache_get_stats( cache, &stats);
    TEST_ASSERT( stats.entries_cached == 1,
                "unreferenced flushed entry (id_a) was freed; "
                "referenced one (id_b) kept");
    TEST_ASSERT( stats.entries_dirty == 0,
                 "no dirty entries remain after sync");
...
static bool test_kes_cache_start(void) {
    ...
    default_config( &cfg);
    /* short interval so the test doesn't stall */
    cfg.sync_interval_ms = 200;
    cache = kes_cache_create( &cfg);
    kes_cache_set_io_callbacks( cache, mock_read, mock_write, mock_sync);
    kes_cache_get_extent( cache, &id, &buf);
    kes_cache_mark_dirty( cache, &id);
