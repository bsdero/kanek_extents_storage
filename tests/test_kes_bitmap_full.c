/*
 * test_kes_bitmap_full.c - Full public API coverage for kes_bitmap.h
 *
 * One demo test per public function in kes_bitmap.h, covering at
 * least one success path and (where the function has one) at least
 * one documented failure path. Added per debugging_plan.md section 6.
 */
...
static bool test_kes_bitmap_create(void) {
    ...
    TEST_ASSERT(kes_bitmap_create(100, &bm) == KES_SUCCESS, "create success");
...
static bool test_kes_bitmap_set(void) {
    ...
    TEST_ASSERT(kes_bitmap_set(bm, 5) == KES_SUCCESS, "re-set already-set bit");
    kes_bitmap_get_stats(bm, NULL, &free_after, NULL);
    TEST_ASSERT(free_after == free_before - 1, "free_bits unchanged on re-set");
...
static bool test_kes_bitmap_clear(void) {
    ...
    kes_bitmap_get_stats(bm, NULL, &free_after, NULL);
    TEST_ASSERT(free_after == free_before + 1, "free_bits unchanged on re-clear");
...
static bool test_kes_bitmap_test(void) {
    ...
    TEST_ASSERT(kes_bitmap_test(NULL, 0) == false, "NULL bitmap reads false");
...
static bool test_kes_bitmap_clear_range(void) {
    ...
    TEST_ASSERT(kes_bitmap_clear_range(bm, 10, 5) == KES_SUCCESS,
                "clear range 10..14");
    for (uint64_t i = 10; i < 15; i++) {
        TEST_ASSERT(kes_bitmap_test(bm, i) == false, "bit in range cleared");
    }
...
static bool test_kes_bitmap_find_free(void) {
    ...
    TEST_ASSERT(kes_bitmap_find_free(bm, 4, 0, &found) == KES_SUCCESS,
                "find 4 free bits");
    TEST_ASSERT(found == 10, "first gap starts right after the occupied range");
...
static bool test_kes_bitmap_get_stats(void) {
    ...
    TEST_ASSERT(kes_bitmap_get_stats(bm, &total, &free_bits, &used) == KES_SUCCESS,
                "get_stats success");
    ...
    TEST_ASSERT(kes_bitmap_get_stats(NULL, &total, NULL, NULL) == KES_ERROR_INVALID,
                "NULL bitmap rejected");
...
static bool test_kes_bitmap_save_load(void) {
    ...
    TEST_ASSERT(kes_bitmap_load(bm_b, fd, 0) == KES_SUCCESS, "load into bm_b");
