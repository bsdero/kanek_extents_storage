/*
 * test_kes_storage_full.c - Full public API coverage for kes_storage.h
 *
 * One demo test per public function in kes_storage.h. Assumes fixes
 * #1, #3, and #7 from debugging_plan.md are already applied -- the
 * KES_ERROR_BUSY string check needs fix #1, and the extent-size math
 * assumes fix #7's overflow correction. This file does not duplicate
 * fix #3's double-free test (that lives in tests/test_kes_minimal.c).
 */
...
static bool test_kes_storage_create(void) {
    ...
    cleanup();
    TEST_ASSERT(make_storage(TEST_FILE_A, &st) == KES_SUCCESS, "create success");
...
static bool test_kes_storage_open_close(void) {
    ...
    TEST_ASSERT(kes_storage_open(TEST_FILE_A, 0, &st) == KES_SUCCESS,
                "re-open success");
    TEST_ASSERT(kes_storage_close(st) == KES_SUCCESS, "close after open success");

    TEST_ASSERT(kes_storage_open(NULL, 0, &st) == KES_ERROR_INVALID,
                "NULL device_path rejected");
    TEST_ASSERT(kes_storage_open("/nonexistent/path/x", 0, &st) == KES_ERROR_IO,
                "nonexistent file reports IO error");

    TEST_ASSERT(kes_storage_close(NULL) == KES_ERROR_INVALID, "close(NULL) rejected");
...
static bool test_kes_storage_open_corrupt(void) {
    ...
    /* leave the descriptor region zeroed -- magic won't match KES_MAGIC_NUMBER */
    close(fd);
...
static bool test_kes_storage_sync(void) {
    ...
    TEST_ASSERT(kes_storage_sync(st) == KES_SUCCESS, "sync writable storage");
    TEST_ASSERT(kes_storage_sync(NULL) == KES_ERROR_INVALID, "NULL storage rejected");
    kes_storage_close(st);

    TEST_ASSERT(kes_storage_open(TEST_FILE_A, KES_STORAGE_READONLY, &st) == KES_SUCCESS,
                "re-open readonly");
...
static bool test_kes_extent_allocate(void) {
    ...
    TEST_ASSERT(kes_extent_allocate(st, &bad_req, &ext) == KES_ERROR_INVALID,
    ...
    TEST_ASSERT(kes_extent_allocate(st, &huge_req, &ext) == KES_ERROR_NOSPACE,
...
static bool test_kes_extent_free(void) {
    ...
    cleanup();
    TEST_ASSERT(make_storage(TEST_FILE_A, &st) == KES_SUCCESS, "create");
    TEST_ASSERT(kes_extent_allocate(st, &req, &ext) == KES_SUCCESS, "allocate");
...
static bool test_kes_extent_read_write(void) {
    ...
    TEST_ASSERT(kes_extent_write(st, &ext, wbuf, sizeof(wbuf), 0) == KES_SUCCESS,
                "write success");
    TEST_ASSERT(kes_extent_read(st, &ext, rbuf, sizeof(rbuf), 0) == KES_SUCCESS,
                "read success");
    TEST_ASSERT(memcmp(wbuf, rbuf, sizeof(wbuf)) == 0, "read matches write");
    ...
    TEST_ASSERT(kes_extent_read(st, &ext, rbuf, sizeof(rbuf), 200000) == KES_ERROR_INVALID,
                "read past extent end rejected");
    TEST_ASSERT(kes_extent_write(st, &ext, wbuf, sizeof(wbuf), 200000) == KES_ERROR_INVALID,
                "write past extent end rejected");

    TEST_ASSERT(kes_extent_read(NULL, &ext, rbuf, sizeof(rbuf), 0) == KES_ERROR_INVALID,
                "read: NULL storage rejected");
    TEST_ASSERT(kes_extent_read(st, &ext, NULL, sizeof(rbuf), 0) == KES_ERROR_INVALID,
                "read: NULL buffer rejected");
    TEST_ASSERT(kes_extent_write(st, &ext, NULL, sizeof(wbuf), 0) == KES_ERROR_INVALID,
                "write: NULL buffer rejected");

    kes_storage_close(st);
    TEST_ASSERT(kes_storage_open(TEST_FILE_A, KES_STORAGE_READONLY, &st) == KES_SUCCESS,
                "re-open readonly");
    TEST_ASSERT(kes_extent_write(st, &ext, wbuf, sizeof(wbuf), 0) == KES_ERROR_INVALID,
...
static bool test_kes_storage_get_stats(void) {
    ...
    TEST_ASSERT(kes_storage_get_stats(st, &stats) == KES_SUCCESS, "get_stats success");
    TEST_ASSERT(stats.used_blocks == 3, "used_blocks reflects allocation");
    TEST_ASSERT(stats.allocated_extents == 1, "allocated_extents reflects allocation");
...
static bool test_kes_storage_get_descriptor(void) {
    ...
    TEST_ASSERT(kes_storage_get_descriptor(NULL, &desc) == KES_ERROR_INVALID,
                "NULL storage rejected");
...
static bool test_kes_get_error_string(void) {
    ...
    TEST_ASSERT(strcmp(kes_get_error_string(KES_ERROR_BUSY), "Resource busy") == 0,
                "KES_ERROR_BUSY string correct (added by debugging_plan.md fix #1)");
...
static bool test_kes_calculate_blocks_needed(void) {
    TEST_ASSERT(kes_calculate_blocks_needed(8192, 4096) == 2, "exact multiple");
    TEST_ASSERT(kes_calculate_blocks_needed(4097, 4096) == 2, "remainder rounds up");
    TEST_ASSERT(kes_calculate_blocks_needed(0, 4096) == 0, "zero bytes needs zero blocks");
...
static bool test_kes_calculate_extent_size(void) {
    ...
    TEST_ASSERT(kes_calculate_extent_size(&ext) == 3 * KES_DEFAULT_BLOCK_SIZE,
                "size == block_count * KES_DEFAULT_BLOCK_SIZE");
    TEST_ASSERT(kes_calculate_extent_size(NULL) == 0, "NULL extent returns 0");
