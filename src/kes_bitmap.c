/*
 * kes_bitmap.c - KANEK Extents Storage Bitmap Implementation
 *
 * This file implements efficient bitmap operations for tracking free
 * and used blocks. Uses optimized bit manipulation for performance.
 *
...
int kes_bitmap_set(kes_bitmap_t* bitmap, uint64_t bit_index) {
    if (!bitmap || bit_index >= bitmap->total_bits) {
        return KES_ERROR_INVALID;
    }
...
int kes_bitmap_set_range(kes_bitmap_t* bitmap, uint64_t start_bit,
                         uint32_t bit_count) {
    if (!bitmap || start_bit >= bitmap->total_bits ||
...
int kes_bitmap_find_free(kes_bitmap_t* bitmap, uint32_t bit_count,
    ...
    if (!bitmap || bit_count == 0 || !found_start) {
        return KES_ERROR_INVALID;
    }
